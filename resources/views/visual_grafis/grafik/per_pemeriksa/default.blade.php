                    ...
                    <div class="d-flex justify-content-between mt-5 mb-0">
                        ...
                        <div class="d-flex">
                            ...
                            <a class="btn btn-primary"
                               href="{{ route('visual_grafis.tabulasi.perpemeriksa', [
                                                           'waktu' => $waktu,
                                                           'unitkerja' => $unitkerja,
                                                           'bidang_id' => $bidang_id
                                                           ]) }}"
    ...
    <script>
        $(function () {
            ...
            const chart = Highcharts.chart('grafik', {
                    ...
                    title: {
                        text: 'Statistik {{ ucfirst($waktu) }} Per Pemeriksa Bidang {{ \App\Bidang::find($bidang_id)->singkatan }}'
                    },
                    @if( $waktu == 'harian')
                    subtitle: {
                    ...
                    series: [{
                        color: '#5232ff',
                        name: "{{ \App\Bidang::find($bidang_id)->singkatan }}",
                        data: [
                                @if (!empty($takahPerPemeriksa))
                                @foreach ($takahPerPemeriksa as $index => $result)
                            {
                                name: `{{$index}}`, y:{{$result}} },
                            @endforeach
                            @endif
